...
try {
    ...
    $stmt2 = $conn->prepare("
        UPDATE employees
        SET fullname = ?, dob = ?, phone = ?, address = ?, race = ?, identity_number = ?, gender = ?, hire_date = ?, department = ?, job_title = ?
        WHERE user_id = ?
    ");
    $stmt2->bind_param("ssssssssssi", $fullname, $dob, $phone, $address, $race, $identityNumber, $gender, $hireDate, $department, $jobTitle, $userId);
    $stmt2->execute();
