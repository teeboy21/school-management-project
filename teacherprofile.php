...
<main class="app-main">
        <header class="app-header">
            ...
            <div class="app-header-meta">
                <div class="app-header-title">Teacher Profile</div>
                <div class="app-header-subtitle">Your profile details, assigned subjects, and class allocations.</div>
            </div>
            <div class="app-user">
        ...
        <section class="app-content">
            <div class="panel" style="margin-bottom:20px;">
                ...
                <div class="info-item"><label>Name</label><p><?= htmlspecialchars($teacher['fullname'] ?? '') ?></p></div>
                <div class="info-item"><label>Email</label><p><?= htmlspecialchars($teacher['email'] ?? '') ?></p></div>
                <div class="info-item"><label>Employee No.</label><p><?= htmlspecialchars($teacher['employee_number'] ?? '') ?></p></div>
                <div class="info-item"><label>Gender</label><p><?= htmlspecialchars($teacher['gender'] ?? '') ?></p></div>
                <div class="info-item"><label>Hire Date</label><p><?= htmlspecialchars($teacher['hire_date'] ?? '') ?></p></div>
                <div class="info-item"><label>Phone</label><p><?= htmlspecialchars($teacher['phone'] ?: '—') ?></p><button class="edit-btn" onclick="openEditModal('phone')">Edit</button></div>
                <div class="info-item"><label>Address</label><p><?= htmlspecialchars($teacher['address'] ?: '—') ?></p><button class="edit-btn" onclick="openEditModal('address')">Edit</button></div>
            </div>
            <div class="panel" style="margin-bottom:20px;">
            ...
            <div class="panel">
                ...
                <?php if ($classes->num_rows > 0): ?>
                    <?php while ($class = $classes->fetch_assoc()): ?>
                        <span class="badge"><?= htmlspecialchars($class['grade'] . ' ' . $class['class_name']) ?></span>
                    <?php endwhile; ?>
                <?php else: ?>
...
function openEditModal(field) {
    ...
    const current = <?= json_encode($teacher) ?>;
